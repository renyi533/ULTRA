...
class DirectLabelMTLSepFeed(BaseInputFeed):
    ...
    def prepare_true_labels_with_index(
        ...
        sum_label_list = 0
        for j in range(0, self.rank_list_size):
            label_list[j] = [label_list[j]/2, label_list[j]/2]
            sum_label_list += sum(label_list[j])

        # Check if data is valid
        #if check_validation and sum(label_list) == 0:
        if check_validation and sum_label_list == 0:
            return
