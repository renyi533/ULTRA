...
class ClickModel:
    def __init__(self, neg_click_prob=0.0, pos_click_prob=1.0,
        ...
        self.setEpsilon()
        self.setClickProb(max_position,
            relevance_grading_num)
        self.setUnbiasedWatchtimeDist(max_position, watchtime_epsilon, relevance_grading_num)
    ...
    # Inspired by ERR
    def setClickProb(self, max_position, 
                     relevance_grading_num):
        b = 1 / \
            (pow(2, relevance_grading_num) - 1)
        self.click_prob = []
        for pos in range(max_position):
            #epsilon_minus = self.epsilon_minus[pos] / (self.epsilon_minus[pos] + self.epsilon_plus[pos])
            #epsilon_plus = self.epsilon_plus[pos] / (self.epsilon_minus[pos] + self.epsilon_plus[pos])
            #self.click_prob.append([epsilon_minus + epsilon_plus * pow(2, i) * b for i in range(relevance_grading_num + 1)])
            #self.click_prob.append([self.epsilon_minus[pos] + self.epsilon_plus[pos] * pow(2, i) * b for i in range(relevance_grading_num + 1)])
            click_prob = []
            for i in range(relevance_grading_num+1):
                relevance_prob = (pow(2, i)-1) * b
                click_prob.append(self.epsilon_minus[pos] * (1-relevance_prob) + self.epsilon_plus[pos] * relevance_prob)
            self.click_prob.append(click_prob)
            #self.click_prob.append([(self.epsilon_minus[pos] * (1-pow(2, i) * b) + self.epsilon_plus[pos] * pow(2, i) * b) for i in range(relevance_grading_num + 1)])

    def setUnbiasedWatchtimeDist(self, max_position, watchtime_epsilon, relevance_grading_num):
        # mean and std of watchtime
        self.unbiased_watchtime_mean = []
        for pos in range(max_position):
            self.unbiased_watchtime_mean.append([self.epsilon_minus[pos] + self.epsilon_plus[pos] * i for i in range(relevance_grading_num + 1)])
...
class PositionBiasedModel(ClickModel):
    ...
    def setExamProb(self, eta):
        self.eta = eta
        self.original_exam_prob = [1, 0.68, 0.64, 0.62, 0.61, 0.605, 0.6, 0.595, 0.59, 0.585] 
        self.exam_prob = [pow(x, eta) for x in self.original_exam_prob]

    def setEpsilon(self):
        self.epsilon_plus = [1, 0.996, 0.99, 0.985, 0.98, 0.975, 0.97, 0.965, 0.96, 0.955]
        self.epsilon_minus = [0.8, 0.26, 0.12, 0.08, 0.06, 0.05, 0.045, 0.04, 0.035, 0.03]
    ...
    def sampleClick(self, rank, relevance_label):
        ...
        click_p = self.click_prob[rank if rank < len(self.click_prob) else -1] \
            [relevance_label if relevance_label < len( \
            self.click_prob[rank]) else -1]
    ...
    def sampleWatchTime(self, rank, relevance_label):
        ...
        while watchtime_bias < 0:
            watchtime_bias = self.getWatchtimeBias(rank)
        unbiased_watchtime = self.getUnbiasedWatchtime(rank, relevance_label)
    ...
    def getUnbiasedWatchtime(self, rank, relevance_label):
        """ get bias value of watchtime based on the rank """

        unbiased_watchtime_mean = self.unbiased_watchtime_mean[rank if rank < len(self.unbiased_watchtime_mean) else -1] \
            [relevance_label if relevance_label < len(self.unbiased_watchtime_mean[rank]) else -1]
