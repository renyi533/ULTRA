...
class NavieMTLMergeAlgorithm(BaseAlgorithm):
    ...
    def __init__(self, data_set, exp_settings, forward_only=False):
        ...
        self.hparams = ultra.utils.hparams.HParams(
            ...
            loss_func='mse_loss',            # Select Loss function
        ...
        if not forward_only:
            ...
            train_outputs = self.ranking_model(
                self.rank_list_size, scope='ranking_model', forward_only=forward_only) # [[?, rank_list_size], [?, rank_list_size]]
            for task_idx in range(0, len(self.hparams.tasks)):
                if task_idx == 0:
                    train_output = train_outputs[task_idx]
                else:
                    train_output += train_outputs[task_idx]
            train_labelss = self.labels[:self.rank_list_size] # [rank_list_size, ?, task_num]
            train_labels = tf.reduce_sum(train_labelss, axis=-1) # [rank_list_size, ?]
