Data_path="/workspace/user_code/violatang/l2-paper/movielens_bpr"   ## Data path where to unzip the data
...
prefix="ml1m_"                       ## name before data, for example setl.train.txt, prefix=set1.
cd ../../
...
test_name=$Data_path/$Data_folder/${prefix}test.txt
if [ ! -f "$test_name" ]
then
    echo "no test, we will split trian with default rate"
    mv $Data_path/$Data_folder/${prefix}train.txt $Data_path/$Data_folder/${prefix}train_orig.txt
    python ./libsvm_tools/split_libsvm_data.py $Data_path/$Data_folder/${prefix}train_orig.txt ${test_name} $Data_path/$Data_folder/${prefix}train.txt 0.2
fi
...
mv $Data_path/cleaned_data/*.txt $Data_path/normalized/
